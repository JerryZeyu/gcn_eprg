...
import pandas as pd
import numpy as np
PATH_INPUT = '/home/zeyuzhang/PycharmProjects/BERT_BERTPK_train_step1_output'
PATH_OUTPUT = '/home/zeyuzhang/PycharmProjects/gcn_eprg/processed_data/worldtree'
PATH_LEMMA_FILE = '/home/zeyuzhang/PycharmProjects/lemmatization-en.txt'
PATH_OUTPUT_FINAL = '/home/zeyuzhang/PycharmProjects/gcn_eprg/processed_data/worldtree_final'
...
def build_adj_matrix(data_path):
    print(11)
    for filename in sorted(glob.glob(os.path.join(data_path,'train', '*.cites'))):
        basename = os.path.basename(filename).split('.')[0]
        print(basename)
        cites_list = []
        idx_list = set()
        with open(os.path.join(data_path, filename), 'r') as file:
            ...
            with open(os.path.join(data_path,'train', basename+'.graph'), 'wb') as fout:
...
def main():
    ...
    if not os.path.exists(PATH_OUTPUT):
        ...
        for folder in ['1', '2', '3', '4', '5']:
            load_plain_text(os.path.join(PATH_INPUT, 'train_'+str(folder)+'_step1_output'), PATH_OUTPUT, top_k)
